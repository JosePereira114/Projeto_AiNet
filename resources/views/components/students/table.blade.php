    ...
    <table class="table-auto border-collapse">
        ...
        <tbody>
            ...
            <tr class="border-b border-b-gray-400 dark:border-b-gray-500">
                ...
                @can('view', $student)
                <td>
                    <x-table.icon-show class="ps-3 px-0.5" href="{{ route('students.show', ['student' => $student]) }}" />
                </td>
                @else
                <td></td>
                @endcan
                ...
                @can('update', $student)
                <td>
                    <x-table.icon-edit class="px-0.5" href="{{ route('students.edit', ['student' => $student]) }}" />
                </td>
                @else
                <td></td>
                @endcan
                ...
                @can('delete', $student)
                <td>
                    <x-table.icon-delete class="px-0.5" action="{{ route('students.destroy', ['student' => $student]) }}" />
                </td>
                @else
                <td></td>
                @endcan
