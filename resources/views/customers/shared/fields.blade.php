...
<div class="flex flex-wrap space-x-8">
    <div class="grow mt-6 space-y-4">
        ...
        <x-field.input name="payment_ref" label="Payment_ref" :readonly="$readonly" value="{{ old('payment_ref', $customer->payment_ref) }}" />
    </div>
    <div class="pb-6">
        <x-field.image name="photo_filename" label="Photo" width="md" :readonly="$readonly" deleteTitle="Delete Photo" :deleteAllow="($mode == 'edit') && (optional($customer->user)->photo_filename)" deleteForm="form_to_delete_photo" :imageUrl="$imageUrl" />
    </div>
</div>